...
interface AztecAstrologyAnalyzerProps {
  contacts: Contact[];
  onGenerateReport: (contact: Contact) => void;
  generatingForIds: string[];
}

const AztecAstrologyAnalyzer: React.FC<AztecAstrologyAnalyzerProps> = ({ contacts, onGenerateReport, generatingForIds }) => {
  const [selectedContactId, setSelectedContactId] = useState<string>('');
  
  const selectedContact = contacts.find(c => c.id === selectedContactId);
  const isGenerating = selectedContact && generatingForIds.includes(selectedContact.id);
  const reportData = selectedContact?.report?.whyTheyAreLikeThis?.aztecAstrology;

  return (
    <div className="p-4 md:p-8">
        <div className="text-center mb-12">
            <h1 style={{fontFamily: 'var(--font-serif)'}} className="text-3xl md:text-4xl font-bold gradient-text mb-2 uppercase tracking-widest">Aztec Astrology</h1>
            <p className="text-[var(--color-text-secondary)] max-w-2xl mx-auto">Analyze an individual's blueprint based on the ancient wisdom of the Tonalamatl calendar.</p>
        </div>
        ...
        <div className="max-w-3xl mx-auto">
            {!selectedContact && (
                <div className="text-center p-12 glass-card">
                    <p className="text-[var(--color-text-secondary)]">Please select a contact to view their Aztec Astrology Blueprint.</p>
                </div>
            )}
            ...
            {selectedContact && !isGenerating && !reportData && (
                <div className="text-center p-12 glass-card">
                    ...
                    <p className="text-[var(--color-text-secondary)] mb-6">An Executive Blueprint must be generated to view their Aztec Astrology.</p>
                    <button
            ...
            {reportData && !isGenerating && (
                <AztecAstrologyChart data={reportData} />
            )}
        </div>
    </div>
  );
...
export default AztecAstrologyAnalyzer;