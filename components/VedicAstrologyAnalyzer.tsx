...
interface VedicAstrologyAnalyzerProps {
  contacts: Contact[];
  onGenerateReport: (contact: Contact) => void;
  generatingForIds: string[];
}

const VedicAstrologyAnalyzer: React.FC<VedicAstrologyAnalyzerProps> = ({ contacts, onGenerateReport, generatingForIds }) => {
  const [selectedContactId, setSelectedContactId] = useState<string>('');
  
  const selectedContact = contacts.find(c => c.id === selectedContactId);
  const isGenerating = selectedContact && generatingForIds.includes(selectedContact.id);
  const reportData = selectedContact?.report?.whyTheyAreLikeThis?.vedicAstrology;

  return (
    <div className="p-4 md:p-8">
        <div className="text-center mb-12">
            <h1 style={{fontFamily: 'var(--font-serif)'}} className="text-3xl md:text-4xl font-bold gradient-text mb-2 uppercase tracking-widest">Vedic Astrology (Jyotish)</h1>
            <p className="text-[var(--color-text-secondary)] max-w-2xl mx-auto">Analyze an individual's blueprint based on the ancient "science of light" to understand their karmic path and destiny.</p>
        </div>

        <div className="max-w-md mx-auto mb-8">
            <label htmlFor="contact-select" className="block text-sm font-medium text-[var(--color-text-secondary)] mb-2 text-center">Select a Profile to Analyze</label>
            <select
                id="contact-select"
        ...
        <div className="max-w-3xl mx-auto">
            {!selectedContact && (
                <div className="text-center p-12 glass-card">
                    <p className="text-[var(--color-text-secondary)]">Please select a contact to view their Vedic Astrology Blueprint.</p>
                </div>
            )}
            ...
            {selectedContact && !isGenerating && !reportData && (
                <div className="text-center p-12 glass-card">
                    ...
                    <p className="text-[var(--color-text-secondary)] mb-6">An Executive Blueprint must be generated to view their Vedic Astrology.</p>
                    <button
            ...
            {reportData && !isGenerating && (
                <VedicAstrologyChart data={reportData} />
            )}
        </div>
    </div>
  );
...
export default VedicAstrologyAnalyzer;