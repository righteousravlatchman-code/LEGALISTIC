...
import type { JapaneseZodiac } from '../types';
import JapaneseZodiacIcon from './JapaneseZodiacIcon';

interface JapaneseZodiacChartProps {
  data: JapaneseZodiac;
}

const JapaneseZodiacChart: React.FC<JapaneseZodiacChartProps> = ({ data }) => {
  return (
    <div tabIndex={0} className="glass-card p-4 md:p-6 transition-all duration-300 flex flex-col md:flex-row items-center gap-6">
      <div className="w-32 h-32 md:w-40 md:h-40 flex-shrink-0 bg-black/30 rounded-full border-2 border-[var(--color-accent-gold)]/50 flex items-center justify-center p-4 shadow-inner">
        <JapaneseZodiacIcon animal={data.animalSign} className="text-[var(--color-accent-gold)] w-full h-full" />
      </div>
      <div className="flex-1 text-center md:text-left">
          <h3 style={{fontFamily: 'var(--font-serif)'}} className="text-lg md:text-xl font-bold uppercase tracking-widest gradient-text">Japanese Zodiac (Jūnishi)</h3>
...
export default JapaneseZodiacChart;