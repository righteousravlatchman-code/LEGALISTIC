...
interface EgyptianAstrologyAnalyzerProps {
  contacts: Contact[];
  onGenerateReport: (contact: Contact) => void;
  generatingForIds: string[];
}

const EgyptianAstrologyAnalyzer: React.FC<EgyptianAstrologyAnalyzerProps> = ({ contacts, onGenerateReport, generatingForIds }) => {
  const [selectedContactId, setSelectedContactId] = useState<string>('');
  
  const selectedContact = contacts.find(c => c.id === selectedContactId);
  const isGenerating = selectedContact && generatingForIds.includes(selectedContact.id);
  const reportData = selectedContact?.report?.whyTheyAreLikeThis?.egyptianAstrology;

  return (
    <div className="p-4 md:p-8">
        <div className="text-center mb-12">
            <h1 style={{fontFamily: 'var(--font-serif)'}} className="text-3xl md:text-4xl font-bold gradient-text mb-2 uppercase tracking-widest">Egyptian Astrology</h1>
            <p className="text-[var(--color-text-secondary)] max-w-2xl mx-auto">Analyze an individual's blueprint based on the ancient wisdom of the Egyptian gods.</p>
        </div>
        ...
        <div className="max-w-3xl mx-auto">
            {!selectedContact && (
                <div className="text-center p-12 glass-card">
                    <p className="text-[var(--color-text-secondary)]">Please select a contact to view their Egyptian Astrology Blueprint.</p>
                </div>
            )}
            ...
            {selectedContact && !isGenerating && !reportData && (
                <div className="text-center p-12 glass-card">
                    ...
                    <p className="text-[var(--color-text-secondary)] mb-6">An Executive Blueprint must be generated to view their Egyptian Astrology.</p>
                    <button
            ...
            {reportData && !isGenerating && (
                <EgyptianAstrologyChart data={reportData} />
            )}
        </div>
    </div>
  );
...
export default EgyptianAstrologyAnalyzer;